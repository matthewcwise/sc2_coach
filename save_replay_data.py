...
def extract_replay_overview(replay_file, personal_account="TheLeengwist"):
    replay = sc2reader.load_replay(replay_file)
    
    # Check if the game is a 1v1
...
def save_replay_data(directory, filepath="data/replay_data.csv", overwrite=True):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".SC2Replay"):
            ...
            replay_summary = extract_replay_overview(full_path)
            
            # Skip non-1v1 games
            if replay_summary is None:
                print(f"Skipping non-1v1 game: {filename}")
                continue
